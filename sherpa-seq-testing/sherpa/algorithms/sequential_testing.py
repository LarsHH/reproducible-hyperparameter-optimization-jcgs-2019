...
class SequentialTesting(Repeat):
    ...
    def __init__(self, algorithm, K, n=(3, 6, 9), P=0.5, alpha=0.05, verbose=False, homoscedastic=True, agg=True, sample_best=False):
        self.algorithm = algorithm
        self.sample = sample_best
        self.K = {1: K}
        self.k = 0
        self.t = 1
        self.n = n
    ...
    def get_best_result(self, parameters, results, lower_is_better):
        ...
        """
        if self.sample:
            return self._get_best_result_sampled(parameters, results, lower_is_better)
        else:
            return self._get_best_observed_result(parameters, results, lower_is_better)
        
        
    def _get_best_observed_result(self, parameters, results, lower_is_better):
        param_names = [p.name for p in parameters]
    ...
    def _get_best_result_sampled(self, parameters, results, lower_is_better):
        param_names = [p.name for p in parameters]
        completed = results.loc[results.Status == TrialStatus.COMPLETED]
        completed.loc[:, 'MeanObjective'] = completed.groupby(param_names)[
            'Objective'].transform('mean')
        
        equivalence_class = completed.loc[completed.stage == completed.stage.max()]
        
        return equivalence_class.loc[:, param_names + ["MeanObjective"]]\
                                .drop_duplicates()\
                                .sample(n=1)\
                                .to_dict('records')[0]
